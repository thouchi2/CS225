...
class BinaryTree
{
  ...
  private:
    ...
    void printPaths(vector <const BinaryTree<T>::Node *> &store, const Node *subRoot) const;
    void printPathsHelper(const Node *subRoot, vector<T> store) const;


    int & sumDistances(int travel,const Node *subRoot, int &total) const;
};
