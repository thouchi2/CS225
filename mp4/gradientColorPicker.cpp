...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
    ...
    int d = abs(xc - x) + abs(yc - y);

    if(d >= rad)
        color = c2;
    else{
        color.red = c1.red - ((d * c1.red)/rad) + ((d * c2.red)/rad);
        color.green = c1.green - ((d * c1.green)/rad) + ((d * c2.green)/rad);
        color.blue = c1.blue - ((d * c1.blue)/rad) + ((d * c2.blue)/rad);
    }
    return color;
}
