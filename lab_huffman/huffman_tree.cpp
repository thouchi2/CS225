...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for(size_t i = 0; i < frequencies.size(); i++){
         singleQueue.push(new TreeNode(frequencies.at(i)));
     }
     ...
     while(mergeQueue.size() + singleQueue.size() != 1){
         ...
         TreeNode *base = new TreeNode(smallest->freq.getFrequency() + smaller->freq.getFrequency());
         base->left = smallest;
         base->right = smaller;
         mergeQueue.push(base);
     }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(!bfile.hasBits())
        ...
        temp1->left = readTree(bfile);
        temp1->right = readTree(bfile);
        return temp1;
    }
}
