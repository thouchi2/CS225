 ...
 */
 ...
 size_t search_idx(const std::vector<T>& elements, const C& val, size_t l, size_t h){
     size_t i = (h + l)/2;
     if(val < elements[i + 1] && val > elements[i])
         return i + 1;
     else if(val == elements[i])
         return i;
     else if(val < elements[i])
         return search_idx(elements, val, l, i);
     else
         return search_idx(elements, val, i + 1, h);
 }

template <class T, class C>
size_t insertion_idx(const std::vector<T>& elements, const C& value)
{
    /* TODO Your code goes here! */
    size_t n = (elements.size());
    if(n == 0 || elements[0] > value)
        return 0;
    else if(elements[n - 1] < value)
        return n;
    else if(elements[n - 1] == value)
        return n - 1;
    else
        return search_idx(elements, value, 0, n - 1);
}
