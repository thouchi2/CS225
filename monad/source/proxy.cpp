...
const char * parse_options(int argc, const char * const * const argv, RunTimeEnvironment::Options & opts)
{
	...
	parseopts.addOption("setpgid", opts.setpgid);
...
int32_t RunTests::run_single_test(const unit_test_input & curr_test)
{
	...
	set_parent_signal_handler(pid);

	int child_status;
	executor.parent();
	auto ws = waitpid(pid, &child_status, 0);
	EXIT_IF_ERROR(ws == -1);
	disable_parent_signal_handler();
...
// Signal handler for the parent proxy: handles SIGTERM and SIGINT
// by killing all processes in the child's process group, followed by
// killing all of the processes in our process group.
pid_t _child_pid = 0;
void parent_signal_handler(int)
{
	if (_child_pid > 0) kill(-_child_pid, SIGKILL);
	kill(-_child_pid, SIGKILL);
	exit(1);
}

void set_parent_signal_handler(pid_t child_pid)
{
	_child_pid = child_pid;

	struct sigaction new_action;
	new_action.sa_handler = proxy::parent_signal_handler;
...
void disable_parent_signal_handler() {
	// doesn't really disable the signal handler, just prevents sending SIGKILL
	// to the child proxy.
	_child_pid = 0;
}
...
void child_timeout_signal_handler(int)
{
	// kill our entire process group
	kill(-1 * getpid(), SIGKILL);
}

// Handler for signals that dump core: we don't want to produce core dumps, so
// we explictly handle those signals.
void child_abort_signal_handler(int signal)
{
	const char *msg = NULL;
	switch (signal) {
		case SIGQUIT: msg = "Terminated by SIGQUIT\n"; break;
		case SIGILL: msg = "Terminated by SIGILL\n"; break;
		case SIGABRT: msg = "Terminated by SIGABRT\n"; break;
		case SIGFPE: msg = "Terminated by SIGFPE\n"; break;
		case SIGSEGV: msg = "Terminated by SIGSEGV\n"; break;
		case SIGBUS: msg = "Terminated by SIGBUS\n"; break;
	}
...
int test_execution::child()
{
	...
	// as the group id
	if (environment.opts.setpgid) {
		setpgid(0, 0);
	}
	// next, we install our signal handlers on SIGALRM and SIGPROF, our two
	// timers
	struct sigaction new_action;
	new_action.sa_handler = proxy::child_timeout_signal_handler;
	sigemptyset(&new_action.sa_mask);
	new_action.sa_flags = 0;
	sigaction(SIGPROF, &new_action, NULL);
	sigaction(SIGALRM, &new_action, NULL);

	// set up handlers for signals that produce coredumps
	new_action.sa_handler = proxy::child_abort_signal_handler;
	//sigaction(SIGQUIT, &new_action, NULL); // SIGQUIT is manual, so don't handle it (dump core)
...
unit_test_result test_execution::result(int32_t return_code)
{
	...
	// since we explictly handle these signals, treat them as the original signal.
	else if (return_code == SIGQUIT ||
	         return_code == SIGILL ||
	         return_code == SIGABRT ||
	         return_code == SIGFPE ||
	         return_code == SIGSEGV ||
	         return_code == SIGBUS)
		return result(Signal(return_code));
	else
...
int32_t fork_execute(F & executor)
{
	...
	{
		set_parent_signal_handler(process_id);
		executor.parent();
...
int test_execution::child_asan()
{
	...
	start_timeout();
	return exec("./proxy-asan", "--nosetpgid", test.name());
}
