...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         * vector as well.
         */
         string nKey = ll.customer.substr(1, ll.customer.size() - 3);
         string nUrl = ll.url;
         nKey.append(nUrl);

         if(whenVisitedTable.keyExists(nKey)){
             if(whenVisitedTable[nKey] < ll.date)
                whenVisitedTable[nKey] = ll.date;
         }
         else
            whenVisitedTable.insert(nKey, ll.date);

        if(!pageVisitedTable.keyExists(nUrl))
            uniqueURLs.push_back(nUrl);
        pageVisitedTable[nUrl] = true;
    }
    infile.close();
}
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
     ...
     */
     string nKey = customer.substr(1, customer.size() - 3);
     string nUrl = url;
     nKey.append(nUrl);
     return whenVisitedTable.keyExists(nKey);
}
                                  ...
                                  const string& url) const
{
     ...
     */
     ...
     string nUrl = url;
     nKey.append(nUrl);
     return whenVisitedTable.find(nKey);
}
