                                 ...
                                 int tolerance, int frameFreq)
{
     ...
     */
    solidColorPicker fill = solidColorPicker(fillColor);
    return filler::fill <Stack> (img, x, y, fill, tolerance, frameFreq);
}
                                ...
                                int gridSpacing, int tolerance, int frameFreq)
{
     ...
     * correct call to fill with the correct colorPicker parameter.
     */
    gridColorPicker fill = gridColorPicker(gridColor, gridSpacing);
    return filler::fill <Stack> (img, x, y, fill, tolerance, frameFreq);
}
                                    ...
                                    int radius, int tolerance, int frameFreq)
{
     ...
     * correct call to fill with the correct colorPicker parameter.
     */
    gradientColorPicker fill = gradientColorPicker(fadeColor1, fadeColor2, radius, x, y);
    return filler::fill <Stack> (img, x, y, fill, tolerance, frameFreq);
}
                                 ...
                                 int tolerance, int frameFreq)
{
     ...
     */
    solidColorPicker fill = solidColorPicker(fillColor);
    return filler::fill <Queue> (img, x, y, fill, tolerance, frameFreq);
}
...
//helper function
int filler::calcT(PNG& img, RGBAPixel original, int storeX, int storeY){
    int rt = ((img(storeX, storeY)->red - original.red)*(img(storeX, storeY)->red - original.red));
    int bt = ((img(storeX, storeY)->blue - original.blue)*(img(storeX, storeY)->blue - original.blue));
    int gt = ((img(storeX, storeY)->green - original.green)*(img(storeX, storeY)->green - original.green));
    int newT = rt + bt + gt;
    return newT;
}
                       ...
                       int tolerance, int frameFreq)
{
     ...
     */
     ...
     bool **checked = new bool*[(int)img.width()];
     for(int i = 0; i < (int)img.width(); i++){
         checked[i] = new bool[(int)img.height()];
     }
     ...
     while(!xcoord.isEmpty() && !ycoord.isEmpty()){
         int storeX = xcoord.remove();
         int storeY = ycoord.remove();
         int newT = calcT(img, origin, storeX, storeY);

         if(tolerance >= newT && checked[storeX][storeY] == false){
             *img(storeX, storeY) = fillColor(storeX, storeY);
             frames++;

             if(frames % frameFreq == 0)
                pic.addFrame(img);

            if((storeX + 1) >= 0 && (storeX + 1) <= ((int)(img.width()) - 1) && storeY >= 0 && storeY <= ((int)img.height() - 1) && checked[storeX + 1][storeY] == false){
                xcoord.add(storeX + 1);
                ycoord.add(storeY);
            }

            if((storeX) >= 0 && storeX <= ((int)img.width() - 1) && (storeY + 1) >= 0 && (storeY + 1) <= ((int)img.height() - 1) && checked[storeX][storeY+1] == false){
                xcoord.add(storeX);
                ycoord.add(storeY + 1);
            }
