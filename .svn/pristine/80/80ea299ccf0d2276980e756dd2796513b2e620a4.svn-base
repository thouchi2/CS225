...
class KDTree
{
  ...
  private:
     ...
     */
     int getDistance(Point<Dim> one, Point<Dim> two) const;

     void buildTree(int dim, int l, int r);

     void quickS(int l, int r, int d, int m);

     int part(int l, int r, int pivIndex, int d);

     Point<Dim>findNearestNeighbor(const Point<Dim> & target, const Point<Dim> & curr,int d, int l, int r) const;
};
