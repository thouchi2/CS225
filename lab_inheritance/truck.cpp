...
void Truck::draw(PNG * page) const{
  engine->Rectangle::draw(page);
  cabin->Rectangle::draw(page);
  window->Rectangle::draw(page);
  trailer->Rectangle::draw(page);

  for(int i = 0; i < NUM_WHEELS; i++){
    wheels[i]->Circle::draw(page);
  }
}
